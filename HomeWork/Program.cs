...
class Program
{
    public static List<Debtor> debtors = new List<Debtor> {
            new Debtor("Ssshirley T. Qqqualls", DateTime.Parse("March 30, 1932"), "530-662-7732", "ShirleyTQualls@teleworm.us", "3565 Eagles Nest Drive Woodland, CA 95695", 2414),
    ...
    static void Main()
    {
        ...
        //var result = debtors.Where(d => d.Email.ToLower().
        //EndsWith("@rhyta.com") || d.Email.ToLower().EndsWith("@dayrep.com"));

        //-----------------------------------------------------------------------

        //2) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag

        //debtors.AgeRangeDebtor(26,36); 
        // MyExtensionda
        ...
        //debtors.RangeDebtor(5000);
        // MyExtensionda

        //-----------------------------------------------------------------------

        //4) Butov adi 18 sivoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq

        //debtors.Symbol(18,7,2);
        // symbolCount,number,numberCount // MyExtensionda

        //-----------------------------------------------------------------------

        //5) Qishda anadan olan borclulari cixardmaq

        //Qis Aylari -> Dekabr(12) , Yanvar(1) , Fevral (2)
        ...
        //var avg = debtors.Average(d => d.Debt); // umumi borclarin orta borcu

        //var result = debtors.Where(d => d.Debt > avg); //umumi borclarin orta borcunnan cox olan borclular

        //var OrderSurname = debtors.OrderBy(d => d.FullName.Split().Last()).ToList(); // Familya Gore sortirovka

        //var OrderDebet = result.OrderByDescending(r => r.Debt); // Meblegin Azalmagina Gore

        //foreach (var debtor in OrderSurname) Console.WriteLine(debtor.ToString());
        //foreach (var debtor in OrderDebet) Console.WriteLine(debtor.ToString());

        //-----------------------------------------------------------------------

        //7) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq

        //var result = debtors.Where(d => !d.Phone.Contains('8')); //Telefon nomresinde 8 olmayan

        //foreach (var debtor in result)
        //{
        //    int AgeCalculator = DateTime.Now.Year - debtor.BirthDay.Year;
        //    Console.WriteLine($"Surname-> {debtor.FullName.Split().Last()} , Age-> {AgeCalculator} , Debt-> {debtor.Debt}");
        //}
        ...
        //for (int i = 0; i < 5; i++) Console.WriteLine(result[i]);
        ...
        //var TheSawWar = debtors.Where(d => d.BirthDay.Year > 1939 && d.BirthDay.Year < 1945);

        //-----------------------------------------------------------------------

        //13)Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq

        //debtors.PhoneNoDuplicates();
        // MyExtensionda
